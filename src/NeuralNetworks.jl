module NeuralNetworks
    ...
    function WideNet(nblocks,layers_pr_block,ch;pixels_in=32,dt=1,nc=10,device=cpu)
        ...
        for i=1:nblocks
            ...
            push!(layers, Chain(BatchNorm(ch[i], σ),SplitConv(Chain(Conv((3,3), ch[i]=>ch[i+1],pad=1,stride=stride),BatchNorm(ch[i+1], σ),Conv((3,3), ch[i+1]=>ch[i+1],pad=1)),dt,Conv((1,1), ch[i]=>ch[i+1],stride=stride)),x -> x[1] .+ x[2]))
            push!(ResLayers,length(layers))
            for j=2:layers_pr_block
                push!(layers, Chain(BatchNorm(ch[i+1], σ),Split(Chain(Conv((3,3), ch[i+1]=>ch[i+1],pad=1),BatchNorm(ch[i+1], σ),Conv((3,3), ch[i+1]=>ch[i+1],pad=1)),dt), x -> x[1] .+ x[2]))
                push!(ResLayers,length(layers))
            end
        end
